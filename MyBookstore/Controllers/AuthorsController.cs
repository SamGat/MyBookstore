...
namespace MyBookstore.Controllers
{
    public class AuthorsController : Controller
    {
                    ...
                    using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                    {
                        using (DataTable dt = new DataTable())
                        {
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                author.Address = row["authorLN"].ToString();
        ...
        public ActionResult Edit(AuthorsModels author)
        {
            ...
            {
                con.Open();
                string query = @"UPDATE authors SET authorLN=@authorLN, authorFN=@authorFN, authorPhone=@authorPhone, authorAddress=@authorAddress, authorCity=@authorCity,
                 authorState=@authorState, authorZip=@authorZip WHERE authorID=@authorID";
